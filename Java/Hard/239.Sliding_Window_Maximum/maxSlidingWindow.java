...
public class maxSlidingWindow {
    ...
    public int[] maxSlidingWindowMaxHeap(int[] nums, int k) {
        ...
        //构建一个max heap
        PriorityQueue<Integer> maxPQ = new PriorityQueue<>((a, b) -> {return b - a;});
        //分配两个指针移动window
        int left = 0, right = 0;
     ...
     *
     * 所以只要用单调deque维持递增序列即可，然后当遍历的window size大于3后，开始将最大的元素（即deque的第一个元素）放入最终结果，同时当左边界移动时，需要将
    ...
    public int[] maxSlidingWindowDeque(int[] nums, int k) {
        ...
        for(int i = 0; i < nums.length; i++) {
            ...
            if(i - k + 1 >= 0) {
                ...
                //若移动左边界越过了当前记录的window左边界，那么需要将deque中的第一个元素（即左边界）从deque中移除
    ...
    @Test
    public void maxSlidingWindowDequeTest() {
        /**
         * Example 1:
