...
public class splitArray {
    ...
    public int splitArrayGreedy(int[] nums, int m) {
        //注意如果对数组元素的和进行累加，可能溢出整型边界，因此将值用long存起来
        long low = -1;
        long high = 0;
        ...
        //最后当前左右边界交叉，该值一定为最小的largest sum，因为先前记为了long，记得转换回int
        return (int) low;
    }
    ...
    public void splitArrayTest() {
        ...
        assertEquals(18, splitArrayDP(nums, 2));
        assertEquals(18, splitArrayGreedy(nums, 2));
    }
}
