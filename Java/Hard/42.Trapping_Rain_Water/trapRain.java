...
public class trapRain {
    ...
    public int trapTwoPointers(int[] height) {
        ...
        while(left < right) {
            if(height[left] < height[right]) { //if left boundary is smaller, it will determine the amount of trapped water
                if(height[left] >= leftMax) {
                    //if current height is larger than the leftMax, update leftMax
                    leftMax = height[left];
                } else {
                    //otherwise, we trapped some water at this index
