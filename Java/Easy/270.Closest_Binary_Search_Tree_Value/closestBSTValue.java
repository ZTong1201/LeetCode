...
public class closestBSTValue {
     ...
     * 在binary search过程中，若当前root值和target的差值小于之前记录的最小diff，则需要不断更新final res和diff值。最终当binary search结束，即
     * 左右边界交叉时（在这题中，就是遍历到null节点），返回记录的res
     * <p>
     * Time: O(H) 因为每次会舍弃掉一半的子树，所以最后只遍历了和树的高度相同的节点
     * Space: O(1) 直接对树本身进行binary search，不需要额外空间
     */
    public int closetValueBinarySearch(TreeNode root, double target) {
        //刚开始可以把res设为root节点值
        int res = root.val;
        //需要在二分查找过程中找到minimum difference
    ...
    public void closestValueTest() {
        ...
        assertEquals(4, closetValueBinarySearch(tree, 3.714286));
    }
