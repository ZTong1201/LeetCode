...
public class BSTToDLL {
    ...
    private void helper(Node root) {
        if(root != null) {
            ...
            if(last != null) {  //which means we have at least one node in our final DLL, link the new node with the last node
                last.right = root;
                root.left = last;
            } else {   //otherwise, there is nothing in the DLL already, simply assign first node to it
                first = root;
            }
