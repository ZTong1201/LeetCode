...
public class wordBreak {
        ...
        for (int start = 0; start < length; start++) {
            for (int end = start + 1; end <= length; end++) {
                if (wordSet.contains(s.substring(start, end))) {
                    // if s[start:end) in the word set
                    // assign dp[start][end - 1] as true
                    dp[start][end - 1] = true;
                    // check whether s[0:start) is also in the word set
                    // if yes, then the entire string s[0:end) is the wordSet
    ...
    public boolean wordBreak1D_DP(String s, List<String> wordDict) {
        ...
        // initialization, assume an empty string is always in the word set
        dp[0] = true;

        for (int end = 1; end <= length; end++) {
            for (int start = 0; start < end; start++) {
                // dp[start] means s[0:start) in the wordSet
                // and if s[start: end) is also in the wordSet
                // then clearly s[0:end) is in the wordSet - dp[end] = true
